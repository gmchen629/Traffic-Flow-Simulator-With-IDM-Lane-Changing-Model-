...
class ShadowCar(implements(Moveable)):
    '''
    A shadow car class implenting the shadow model
    '''

    def __init__(self, x, lane, length=0, duration=0):
        class MicroModelShadow():
            def calc_acc(self, bwd, vwd):
                return 0
        ...
        self.pos, self.lane, self.length = x, lane, length
        self.vel = 0
        self.model = MicroModelShadow()
        # if the traffic flow will return to normal once the anomaly disappears, then constant ≡ 1.
        self.lifecycle = duration * 1
        self.isShadow = True
        self.isNormalCar = False
