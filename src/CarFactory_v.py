...
class CarFactory_v():
    ...
    '''
    def __init__(self, auto_prob, car_prob,v0_car = V0_INIT_CAR_KMH, v0_truck = V0_INIT_TRUCK_KMH):
        self.auto_prob, self.car_prob =  auto_prob, car_prob
        self.vcar, self.vtruck = v0_car,v0_truck

        self.car_human_IDM = IDM(v0 = self.vcar / 3.6,
                                 a  = A_INIT_CAR_MSII,
                                 b  = B_INIT_CAR_MSII,
                                 s0 = S0_INIT_HUMAN,
                                 T  = T_REACTION_HUMAN)

        self.truck_human_IDM = IDM(v0 = self.vtruck  / 3.6,
                                   a  = A_INIT_TRUCK_MSII,
                                   b  = B_INIT_TRUCK_MSII,
                                   s0 = S0_INIT_HUMAN,
                                   T  = T_REACTION_HUMAN)

        self.car_auto_IDM = IDMAuto(v0 = self.vcar / 3.6,
                                    a  = A_INIT_CAR_MSII,
                                    b  = B_INIT_CAR_MSII,
                                    s0 = S0_INIT_AUTO,
                                    T  = T_REACTION_AUTO)

        self.truck_auto_IDM = IDMAuto(v0 = self.vtruck  / 3.6,
                                      a  = A_INIT_TRUCK_MSII,
                                      b  = B_INIT_TRUCK_MSII,
                                      s0 = S0_INIT_AUTO,
                                      T  = T_REACTION_AUTO)
