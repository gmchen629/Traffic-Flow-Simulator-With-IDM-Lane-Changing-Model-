...
class IDM():
    ...
    def calc_acc(self, bwd: Moveable, fwd: Moveable):
        ...
        vel = bwd.vel
        #print("add c...")
        c = 0.25*np.random.randn() + 0.7 #高V0 效果不错
        #c = 1 *np.random.randn() #低V0效果不错

        #c=100 # 非常贴近，没有效果
        s_star_raw = self.s0 + vel * self.T\
                        + (vel * delta_v) / (2 * self.sqrtab) + c*((vel **2)/self.b)
        s_star = max(s_star_raw, self.s0)
        acc = self.a * (1 - np.power(vel / self.v0, self.delta) - (s_star **2) / (s**2)) + c
